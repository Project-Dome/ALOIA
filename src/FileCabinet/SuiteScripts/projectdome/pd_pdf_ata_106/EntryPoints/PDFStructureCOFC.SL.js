...
function (record, log, render, format, file, runtime, search) {
  ...
  function collectLines(invoice, vendorIdFilter, groupFilter) {
    ...
    for (var i = 0; i < lineCount; i++) {
      ...
      // Seriais e Lot/Batch — ambos vêm do inventorydetail subrecord
      var serials = [];
      var lots    = [];
      try {
        ...
        if (invDet) {
          ...
          for (var a = 0; a < assCount; a++) {
            // Use getText on inventoryassignment directly — more reliable than lookupFields
            // for inventorynumber (avoids array-vs-string ambiguity in lookupFields results)
            var serialNum = invDet.getSublistText({
              sublistId: 'inventoryassignment',
              fieldId:   'issueinventorynumber',
              line:      a
            }) || '';
            if (serialNum) serials.push(escapeXml(serialNum));

            // Lot number still needs lookupFields since there's no direct text field on assignment
            ...
            if (issueInvNumId) {
              try {
                ...
                var lot = (sn && sn.lotnumber) ? sn.lotnumber : '';
      ...
      var partNumber = invoice.getSublistValue({ sublistId:'item', fieldId:'custcol_pd_partnumbercustomer_display', line:i }) || '';
      var vendorPartNumber = invoice.getSublistValue({ sublistId:'item', fieldId:'custcol_pd_partnumbervendor', line:i }) || '';
      var poNumber   = invoice.getSublistText({ sublistId:'item', fieldId:'custcol_aae_purchaseorder',  line:i }) || '';
